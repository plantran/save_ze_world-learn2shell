...
$users = [
  ...
  {:name=>"Inès", :blood=>"AB-", :code_cb=>"7584747465657882", :age=>16},
  ...
  {:name=>"Anaïs", :blood=>"B-", :code_cb=>"8874717869668685", :age=>12},
...
class User < FakeDir
  attr_reader :name, :parent_dir, :slug
  attr_accessor :age, :blood, :card, :list
...
class UsersDir
  ...
  def set_list
    list = Array.new
    $users.each do |u|
      list << {name: u[:name], slug: u[:name].downcase, removable: false, locked: false, kind: :dir}
    end
    list
  end
end
...
def new_user_class user
  ...
  klass = Class.new(User) do
    ...
    define_method :initialize do
      ...
      @list = [
        {name: "Prochaine analyse", slug: "prochaine-analyse", removable: true, locked: false, content: "Prochaine analyse: #{rand(t1..t2)}", kind: :file},
        {name: "Dernière analyse", slug: 'derniere-analyse', removable: false, locked: false, content: faker_analyse(user), kind: :file}
      ]
    end
  end
